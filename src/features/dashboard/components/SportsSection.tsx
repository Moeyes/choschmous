  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[
          { label: "Total Sports", value: String(stats.totalSports), color: "bg-purple-100", icon: <Trophy className="h-6 w-6 text-purple-600" /> },
          { label: "Active Sports", value: String(stats.activeSports), color: "bg-green-100", icon: <Trophy className="h-6 w-6 text-green-600" /> },
          { label: "Total Participants", value: String(stats.totalParticipants), color: "bg-blue-100", icon: <Users className="h-6 w-6 text-blue-600" /> },
          { label: "Categories", value: String(stats.categories), color: "bg-orange-100", icon: <Trophy className="h-6 w-6 text-orange-600" /> },
        ].map((s) => (
          ...
          <TableBody>
            {list.map((sport) => (
              <TableRow key={sport.id} className="group transition-colors">
                <TableCell className="font-bold text-slate-700">{sport.name}</TableCell>
                <TableCell className="text-slate-500 font-medium">{sport.category}</TableCell>
                <TableCell className="text-slate-500 font-medium">{sport.participants}</TableCell>
                <TableCell>
                  <span className={`px-3 py-1 rounded-lg text-xs font-medium ${
                    sport.status === "Ongoing" ? "bg-green-100 text-green-700" : 
                    sport.status === "Completed" ? "bg-blue-100 text-blue-700" : 
