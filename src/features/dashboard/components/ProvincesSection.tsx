...
export function ProvincesSection({ provinces, onCreateProvince }: ProvincesSectionProps) {
  ...
  const stats = useMemo(() => {
    ...
    const totalParticipants = list.reduce((s, p) => s + p.participants, 0)
    const avgParticipants = (totalParticipants / Math.max(1, list.length)).toFixed(1)
    return { totalProvinces, totalParticipants, avgParticipants }
  }, [list])
  ...
  return (
    <div className="space-y-6">
      ...
      <StatsGrid
        items={[
          { label: "សរុបខេត្ត", value: String(stats.totalProvinces), color: "bg-blue-100" },
          { label: "សរុបអ្នកចូលរួម", value: String(stats.totalParticipants), color: "bg-green-100" },
          { label: "មធ្យមអ្នកចូលរួម/ខេត្ត", value: String(stats.avgParticipants), color: "bg-orange-100" },
        ]}
      />
            ...
            <div className="flex flex-col h-full justify-center space-y-4">
              ...
              <div className="space-y-3 pt-4 border-t">
                <div className="flex justify-between items-center">
                  <span className="text-slate-500 font-bold">អ្នកចូលរorg:</span>
                  <span className="text-4xl font-black text-[#1a4cd8]">{topProvince.participants}</span>
                </div>
              </div>
            </div>
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-slate-50/50 hover:bg-slate-50/50">
                <TableHead className="font-bold text-[10px] uppercase text-slate-400">ចំណាត់ថ្នាក់</TableHead>
                <TableHead className="font-bold text-[10px] uppercase text-slate-400">ខេត្ត/ក្រុង</TableHead>
                <TableHead className="font-bold text-[10px] uppercase text-slate-400">អ្នកចorg រួម</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {[...list].sort((a, b) => b.participants - a.participants).map((province, index) => (
                <TableRow key={province.name} className="group transition-colors">
                  ...
                  <TableCell className="font-bold text-[#1a4cd8]">{province.participants}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
